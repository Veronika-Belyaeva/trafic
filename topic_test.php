...
// Функция для получения вопросов по идентификатору темы
...
// Получение идентификатора темы из запроса
$topic_id = isset($_GET['topic_id']) ? intval($_GET['topic_id']) : 1;

// Если пользователь выбрал новую тему, сбрасываем ответы на предыдущие вопросы
if (isset($_GET['topic_id']) && $_GET['topic_id'] != $topic_id) {
...
// Получение вопросов для заданной темы
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($current_question_index < $total_questions) {
        header("Location: topic_test.php?topic_id={$topic_id}&question={$current_question_index}");
        exit();
    }
}
...
if ($all_answered) {
  ...
  echo "<p>Вы ответили правильно на {$correct_answers_count} из {$total_questions} вопросов.</p>";
  echo "<a href=\"topic_test.php?topic_id={$topic_id}\">Пройти тест заново</a>";
...
<head>
  ...
  <style>
    .column {
      width: calc(100% / <?php echo $num_columns; ?>); /* Распределяем ширину столбцов */
      float: left; /* Выравниваем столбцы по горизонтали */
    }
  </style>
  <title>Тест по теме "<?php echo htmlspecialchars($questions[0]['topic_name']); ?>"</title>
</head>
...
<div class="main-container">
    ...
    <h1>Тест по теме "<?php echo htmlspecialchars($questions[0]['topic_name']); ?>"</h1>
    <div class="nav" <?php if ($total_questions > 35) echo 'style="display: grid; grid-template-columns: repeat('.ceil($total_questions / 6).', 1fr);"'; ?>>
    <?php foreach ($questions as $index => $question): ?>
        ...
        ?>
        <a href="topic_test.php?topic_id=<?php echo $topic_id; ?>&question=<?php echo $index; ?>" class="<?php echo $class; ?>"><?php echo $index + 1; ?></a>
    <?php endforeach; ?>
    </div>
